...
#   create a new object that manages the list of objects

#NOTE: code lines 1-25, 26-27, 29-34, 36-42, 44-49, 51-58, 60-65, 67-69, 71-78, 80-91, 95-121
#   Ball class
#   create class BallList
#   intialize BallList; attribute w/ empty list
#   add_ball method (add ball to list) & create Ball obj & append to list
#   delete _ball (deletes ball from list) & use find_ball method to find index of ball, then delete from list
#   change_pressure (changes pressure in particular ball) & use find_ball to find index of ball, then adds pressure to ball
#   find_ball (finds ball in list) & loops through BallList and searches for a match in BallType; returns index when found
#   number_of_balls (returns num of balls in list)
#   print_ball_list (prints list and num of balls)
#   add_ball_from_file (reads file & appends values to list) NOTE use of self.add_ball to add the ball
#   main program
